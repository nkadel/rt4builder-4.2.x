...
BuildRequires: perl(FCGI) >= 0.74
BuildRequires: perl(FCGI::ProcManager)
BuildRequires: perl(File::Dropbox)
...
BuildRequires: perl(HTML::FormatText::AndTables) >= 0.06
...
%{?with_graphviz:BuildRequires: perl(IPC::Run) >= 0.90}
...
BuildRequires: perl(Pod::Simple) >= 3.24
...
%{?with_devel_mode:BuildRequires: perl(Test::Builder) >= 0.90}
...
%{?with_devel_mode:BuildRequires: perl(Test::LongString)}
...
%{?with_devel_mode:BuildRequires: perl(WWW::Mechanize) >= 1.52}
BuildRequires: perl(XML::RSS) >= 1.05
%{?with_devel_mode:BuildRequires: perl(XML::Simple)}
%{?with_devel_mode:BuildRequires: perl(XML::RSS) >= 1.05}

%{?with_runtests:BuildRequires: perl(DBD::Pg) >= 1.43}
%{?with_runtests:BuildRequires: perl(DBD::SQLite) >= 1.00}
